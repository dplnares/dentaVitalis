...
class ModelPagos
{
  ...
  {
    $statement = Conexion::conn()->prepare("SELECT tba_pago.IdPago, tba_pago.IdTipoPago, tba_pago.TotalPago, tba_pago.FechaPago, tba_tipodepago.DescripcionTipo FROM $tabla INNER JOIN tba_tipodepago ON tba_pago.IdTipoPago = tba_tipodepago.IdTipoPago INNER JOIN tba_paciente ON tba_pago.IdPaciente = tba_paciente.IdPaciente WHERE tba_pago.IdPaciente = $codPaciente ORDER BY IdPago DESC");
    $statement -> execute();
    return $statement -> fetchAll();
  }
}